...
Job *Management::giveAssignments(GtpConfigElements *config) {

    //Make the OpenCl tuning before starting the threads
    QTextStream(stdout) << "Starting tuning process, please wait..." << endl;

    QString tuneCmdLine("./leelaz --tune-only -m 30 -w ");

    if (config->enable_noise)
        tuneCmdLine.append("-n ");
    if (config->random_seed > 0)
        tuneCmdLine.append("-s " + QString::number(config->random_seed) + " ");
    if (!config->heuristic)
        tuneCmdLine.append("--dumbpass ");
    if (config->random_num > 0)
        tuneCmdLine.append("-m " + QString::number(config->random_num) + " ");

    tuneCmdLine.append("-w " + config->net_path + config->net_file);
    ...
    for (int gpu = 0; gpu < m_gpus; ++gpu) {
        for (int game = 0; game < m_games; ++game) {
            ...
            QString options;
            options.append("-v " + QString::number(config->loop_visits));
            options.append(" -r 5 -t 1 --noponder");
            t["leelazVer"] = "1.15";
            t["rndSeed"] = "";
            t["optHash"] = "ee21";
            t["options"] = options;
            t["debug"] = "false";
            t["network"] = config->net_file;
...
void Management::terminate_leelaz() {
    QProcess *p = (QProcess *)lz_process;
    p->terminate();
    for (int i = 0; i < m_gpus * m_games; ++i) {
        m_gamesThreads[i]->terminate();
        QTextStream(stdout) << "Management: Worker " << i+1 << " ended" << endl;
    }
}
