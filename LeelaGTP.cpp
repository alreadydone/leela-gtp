...
LeelaGTP::LeelaGTP(QApplication *app, QWidget *parent) :
    ...
    tmplabel2->setGeometry(660, 426, 120, 24);
    this->butt_maxgames = new QSpinBox(this);
    this->butt_maxgames->setGeometry(660, 450, 84, 24);
    this->butt_maxgames->setRange(0, 1000000);
    this->butt_maxgames->setValue(0);
    ...
    this->butt_keepSgf->setChecked(true);
    ...
    this->show_netfile->setText("默認文件：" + config.net_path + config.net_file);
...
void LeelaGTP::initial() {
    ...
    /* 加載圖片 */
    /*
    draw_imgbord.load("./images/bord.png");
    draw_imgcur.load("./images/cur_mov.png");
    draw_imgb.load("./images/movblack.png");
    draw_imgw.load("./images/movwhite.png");
    // */
...
void LeelaGTP::on_netfilebutt() {
    ...
    config.net_path = file.filePath();
    show_netfile->setText(config.net_path + config.net_file);
}
...
int LeelaGTP::_run() {
    ...
    std::cout << "before 1st connect\n";
    QTimer *timer = new QTimer();
    Job *job = boss->giveAssignments(&config);
    //connect(job, SIGNAL(Job::sendmessage(int)), this, SLOT(on_recvmove(int)));
    job->connect_sendmessage(this, SLOT(on_recvmove(int)));
    if (config.run_timeout > 0) {
        std::cout << "before 2st connect\n";
        ...
        if (config.run_maxgames > 0) {
            std::cout << "before 3st connect\n";
            connect(boss, &Management::sendQuit, this, &LeelaGTP::on_bossexit);
        } else {
            Console *cons = new Console();
            connect(cons, &Console::sendQuit, this, &LeelaGTP::on_bossexit);
        }
    }
    return 0;
}
...
void LeelaGTP::on_bossexit() {
    ...
    _enable_all_elements(true);
    butt_run->setText("開始訓練");
    std::cout << "before store games\n";
    boss->storeGames();
    std::cout << "after store games\n";
    boss->terminate_leelaz();
    boss->wait();
    delete boss;
    boss = NULL;
}
...
void LeelaGTP::on_recvmove(int move) {
    ...
    switch (cmd) {
    case 0: // move
        draw_mov.configDropStone(move / 10000, move % 10000);
        break;
    case 2: // pass
        return;
    case 1: // init
